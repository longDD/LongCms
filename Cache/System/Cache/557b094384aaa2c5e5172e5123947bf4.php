    ...
    <div class="subMenuList">
        ...
        <ul>
            <?php if(is_array($Smenu)): foreach($Smenu as $key=>$v): ?><li><a href="<?php echo U($v['model'].'/'.$v['action']);?>"><?php echo ($v["name"]); ?></a></li><?php endforeach; endif; ?>
                ...
                <div id="Right">
                    <!-- longdd 2013.7.29 后台菜单-->

<div>
    <div class="Item hr">
        <div class="current">
            <?php if(ACTION_NAME == 'menu_edit'): ?>编辑栏目
                <?php else: ?>
                添加栏目<?php endif; ?>
        </div>
    </div>
    <!-- Add -->
    <div id='Add_user'>
        <form action="<?php if(ACTION_NAME == 'menu_edit'): echo U('Access/menu_edit'); else: echo U('Access/menu_add'); endif; ?>" method="post">
            <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table1">
                <tr>
                    <th width="120">名称：</th>
                    <td>
                        <input type="text" name="name" class="input" value="<?php echo ($info["name"]); ?>"/>
                        <?php if(ACTION_NAME == 'menu_edit'): ?><input type="hidden" name="id" class="input" value="<?php echo ($info["id"]); ?>"/><?php endif; ?>
                    </td>
                </tr>
                <tr>
                    <th width="120">模型：</th>
                    <td>
                        <input type="text" name="model" class="input" value="<?php echo ($info["model"]); ?>"/>
                    </td>
                </tr>
                <tr>
                    <th width="120">操作：</th>
                    <td>
                        <input type="text" name="action" class="input" value="<?php echo ($info["action"]); ?>"/>
                    </td>
                </tr>
                <tr>
                    <th width="120">数据：</th>
                    <td>
                        <input type="text" name="data" class="input" value="<?php echo ($info["data"]); ?>"/>
                    </td>
                </tr>
                <tr>
                    <th>状态：</th>
...
<?php if(ACTION_NAME == 'menu_edit'): ?><script>
        $(function(){
